...
class RTMConnector(Rtm):
    ...
    def __init__(self, perms="read", api_version=None, env_file='./.env'):
        ...
        for var in self._required_env:
            if not var in os.environ:
                raise Exception(f"Missing the {var} environment variable - please update .env")
        ...
        super().__init__(api_key, shared_secret, perms=perms, token=token, api_version=api_version)
    ...
    def _update_connection(self):
        ...
        print(f"Follow the URL to authenticate with the Remember The Milk API:\n\n{url}\n")
